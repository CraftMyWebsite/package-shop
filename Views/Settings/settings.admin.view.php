...
<script>
    ...
    const shopTypeSelected = document.getElementById('shopTypeSelected');

    const messages = {
        virtual: "Vous avez opté pour le mode <strong>Virtuel uniquement</strong>.<br>Ce mode ne nécessite pas d’adresse client, ce qui simplifie le tunnel de commande.",
        both: "Vous avez opté pour le mode <strong>Virtuel et physique</strong>.<br>Ce mode permet de vendre tous types d’articles, nécessite une adresse même pour les produits virtuels.",
        physical: "Vous avez opté pour le mode <strong>Physique uniquement</strong>.<br>Ce mode nécessite une adresse client pour la livraison ou le retrait."
    };

    function updateMessage() {
        const value = shopTypeSelect.value;
        shopTypeSelected.innerHTML = messages[value];
    }

    updateMessage();

    shopTypeSelect.addEventListener('change', updateMessage);
</script>
