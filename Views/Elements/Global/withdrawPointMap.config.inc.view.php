    ...
    <div class="grid-2">
        <div>
            ...
            <select name="<?= $varName ?>_use" id="<?= $varName ?>_use">
                <option <?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_use', $varName) == "1" ? 'selected' : ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_use', $varName) ?? 'selected' ?> value="1">Oui</option>
                <option <?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_use', $varName) == "0" ? 'selected' : '' ?> value="0">Non</option>
            </select>
        </div>
        <div>
            <label for="<?= $varName ?>_client_address">Nom de l'adresse du client dans le popup :</label>
            <input
                value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_client_address', $varName) ?? 'Votre adresse' ?>"
