...
<section>
    ...
    <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_global') ?? 'Carte cadeau' ?>" type="text" name="<?= $varName ?>_global" id="<?= $varName ?>_global" class="input" required>
</section>
<section class="grid-3">
    <div>
        <h5 class="text-center">Paramétrage du mail :</h5>
        <label for="<?= $varName ?>_title_mail">Titre :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_title_mail') ?? 'Félicitations !' ?>" type="text" name="<?= $varName ?>_title_mail" id="<?= $varName ?>_title_mail" class="input" required>
        <label for="<?= $varName ?>_text_mail">Message :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_text_mail') ?? "Vous avez reçu une carte cadeau d'une valeur de" ?>" type="text" name="<?= $varName ?>_text_mail" id="<?= $varName ?>_text_mail" class="input" required>
        <label for="<?= $varName ?>_prefix">Préfixe du code :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_prefix') ?? 'GC_' ?>" type="text" name="<?= $varName ?>_prefix" id="<?= $varName ?>_prefix" class="input" required>
        <label for="<?= $varName ?>_use_mail">Message footer :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_use_mail') ?? 'Utilisez ou partager ce code lors de votre prochain achat sur' ?>" type="text" name="<?= $varName ?>_use_mail" id="<?= $varName ?>_use_mail" class="input" required>
        <label for="<?= $varName ?>_url_mail">Url du site :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_url_mail') ?? Website::getUrl() . 'shop' ?>" type="text" name="<?= $varName ?>_url_mail" id="<?= $varName ?>_url_mail" class="input" required>
        <label for="<?= $varName ?>_time_mail">Temps restant :</label>
        <input value="<?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_time_mail') ?? "Ce code est valable jusqu'au" ?>" type="text" name="<?= $varName ?>_time_mail" id="<?= $varName ?>_time_mail" class="input" required>
    </div>
        ...
        <div class="border dark:border-gray-700">
            ...
            <div style="background-color: <?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_body_color') ?? '#ffffff' ?>">
                <div class="my-gift-card-scope p-4">
                    <div class="gift-card">
                        <h2><?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_title_mail') ?? 'Félicitations !' ?></h2>
                        <p><?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_text_mail') ?? "Vous avez reçu une carte cadeau d'une valeur de" ?> <strong>XX</strong></p>
                        <div class="code"><?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_prefix') ?? 'GC_' ?>XXXXXXXX</div><br>
                        <p style="font-size: 0.8rem"><?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_use_mail') ?? 'Utilisez ou partager ce code lors de votre prochain achat sur' ?> <a href="%URL%"><?= Website::getWebsiteName() ?></a>.<br>
                            <?= ShopSettingsModel::getInstance()->getGlobalSetting($varName . '_time_mail') ?? "Ce code est valable jusqu'au" ?> XX/XX/XX</p>
                    </div>
                </div>
            </div>
        </div>
