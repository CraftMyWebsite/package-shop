...
<section>
    ...
    <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_global") ?? "Carte cadeau"?>" type="text" name="<?= $varName ?>_global" id="<?= $varName ?>_global" class="form-control" required>
</section>
<section class="row mt-2">
    <div class="col-12 col-lg-5">
        <h5 class="text-center">Paramétrage du mail :</h5>
        <label for="<?= $varName ?>_title_mail">Titre :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_title_mail") ?? "Félicitations !" ?>" type="text" name="<?= $varName ?>_title_mail" id="<?= $varName ?>_title_mail" class="form-control" required>
        <label for="<?= $varName ?>_text_mail">Message :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_text_mail") ?? "Vous avez reçu une carte cadeau d'une valeur de" ?>" type="text" name="<?= $varName ?>_text_mail" id="<?= $varName ?>_text_mail" class="form-control" required>
        <label for="<?= $varName ?>_prefix">Préfixe du code :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_prefix") ?? "GC_" ?>" type="text" name="<?= $varName ?>_prefix" id="<?= $varName ?>_prefix" class="form-control" required>
        <label for="<?= $varName ?>_use_mail">Message footer :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_use_mail") ?? "Utilisez ou partager ce code lors de votre prochain achat sur" ?>" type="text" name="<?= $varName ?>_use_mail" id="<?= $varName ?>_use_mail" class="form-control" required>
        <label for="<?= $varName ?>_url_mail">Url du site :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_url_mail") ?? Website::getUrl()."shop" ?>" type="text" name="<?= $varName ?>_url_mail" id="<?= $varName ?>_url_mail" class="form-control" required>
        <label for="<?= $varName ?>_time_mail">Temps restant :</label>
        <input value="<?= ShopItemsVirtualRequirementModel::getInstance()->getGlobalSetting($varName."_time_mail") ?? "Ce code est valable jusqu'au"?>" type="text" name="<?= $varName ?>_time_mail" id="<?= $varName ?>_time_mail" class="form-control" required>
    </div>
